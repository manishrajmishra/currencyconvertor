const addCurrencyBtn = document.querySelector(".add-currency-btnbottom");
const addCurrencyList = document.querySelector(".add-currency-listbottom");
const currenciesList = document.querySelector(".currenciesbottom");
...
function currenciesListClick(event) {
  if(event.target.classList.contains("close")) {
    ...
    if(parentNode.classList.contains("base-currencybottom")) {
      const newBaseCurrencyLI = currenciesList.querySelector(".currencybottom");
      if(newBaseCurrencyLI) {
        setNewBaseCurrency(newBaseCurrencyLI);
        baseCurrencyAmount = Number(newBaseCurrencyLI.querySelector(".inputbottom input").value);
      }
    }
  }
}
...
function setNewBaseCurrency(newBaseCurrencyLI) {
  newBaseCurrencyLI.classList.add("base-currencybottom");
  baseCurrency = newBaseCurrencyLI.id;
  const baseCurrencyRate = currencies.find(currency => currency.abbreviation===baseCurrency).rate;
  currenciesList.querySelectorAll(".currencybottom").forEach(currencyLI => {
    const currencyRate = currencies.find(currency => currency.abbreviation===currencyLI.id).rate;
    const exchangeRate = currencyLI.id===baseCurrency ? 1 : (currencyRate/baseCurrencyRate).toFixed(4);
    currencyLI.querySelector(".base-currency-ratebottom").textContent = `1 ${baseCurrency} = ${exchangeRate} ${currencyLI.id}`;
  });
}
...
function currenciesListInputChange(event) {
  ...
  if(isNewBaseCurrency) {
    currenciesList.querySelector(`#${baseCurrency}`).classList.remove("base-currencybottom");
  ...
  if(baseCurrencyAmount!==newBaseCurrencyAmount || isNewBaseCurrency) {
    ...
    currenciesList.querySelectorAll(".currencybottom").forEach(currencyLI => {
      if(currencyLI.id!==baseCurrency) {
        const currencyRate = currencies.find(currency => currency.abbreviation===currencyLI.id).rate;
        const exchangeRate = currencyLI.id===baseCurrency ? 1 : (currencyRate/baseCurrencyRate).toFixed(4);
        currencyLI.querySelector(".inputbottom input").value = exchangeRate*baseCurrencyAmount!==0 ? (exchangeRate*baseCurrencyAmount).toFixed(4) : "";
      }
    });
  }
}
  ...
  for(let i=0; i<currencies.length; i++) {
    addCurrencyList.insertAdjacentHTML(
      ...
      `<li data-currency=${currencies[i].abbreviation}>
        <img src=${currencies[i].flagURL} class="flagbottom"><span>${currencies[i].abbreviation} - ${currencies[i].name}</span>
      </li>`
    );
  }
}
...
function newCurrenciesListItem(currency) {
  ...
  currenciesList.insertAdjacentHTML(
    "beforeend",
    `<li class="currencybottom ${currency.abbreviation===baseCurrency ? "base-currency" : ""}" id=${currency.abbreviation}>
      <img src=${currency.flagURL} class="flagbottom">
      <div class="infobottom">
        <p class="inputbottom"><span class="currency-symbolbottom">${currency.symbol}</span><input placeholder="0.0000" value=${inputValue}></p>
        <p class="currency-namebottom">${currency.abbreviation} - ${currency.name}</p>
        <p class="base-currency-ratebottom">1 ${baseCurrency} = ${exchangeRate} ${currency.abbreviation}</p>
      </div>
...
fetch(dataURL)
  .then(res => res.json())
  .then(data => {
    document.querySelector(".datebottom").textContent = data.date;
    data.rates["EUR"] = 1;
